...
public class LocalDateTimeUtil{
    ...
    private static final Map<Pattern,String> DATE_FORMATTER_PATTERN_MAP = new HashMap<>();
    static {
        DATE_FORMATTER_PATTERN_MAP.put(Pattern.compile("^\\d{4}$"),YYYY);
        DATE_FORMATTER_PATTERN_MAP.put(Pattern.compile("^\\d{4}-\\d{2}$"),YYYY_MM);
        DATE_FORMATTER_PATTERN_MAP.put(Pattern.compile("^\\d{4}-\\d{2}-\\d{2}$"),YYYY_MM_DD);
        DATE_FORMATTER_PATTERN_MAP.put(Pattern.compile("^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}$"),YYYY_MM_DD_HH_MM_SS);
    }
     ...
     *  startTime 开始时间
     *  endTime   计算时间
     *  field     根据field不同减不同值 , field 为 ChronoUnit.*
     *  startTime小 endTime大 返回正数，则反之
     * @author wangsong
     * <p>
     * 秒    ChronoUnit.SECONDS
     * 分    ChronoUnit.MINUTES
    ...
    public static LocalDateTime weekFirstDay(LocalDateTime time, int num) {
        ...
        LocalDateTime newTime = minus(LocalDateTime.now(), week - 1L, ChronoUnit.DAYS);
    ...
    public static LocalDateTime yearLastDay(LocalDateTime time, int num) {
        LocalDateTime newTime = minus(time, num, ChronoUnit.YEARS);
        int year = newTime.getYear();
        // 年 月  天 时 分 秒 毫秒（这里精确到6位数）
     ...
     *
     *  startNum 前n月，当前月开始为 0
     *  endNum   后n月，当前月就是为 0
     *  java.util.List<com.lplb.common.utils.LocalDateTimeUtil.DateDays>
     * @author wangsong
     */
     ...
     *  </P>
     *
     *  startTime 开始时间 (时分秒已开始时间位为准)
     *  endTime   结束时间
     *  type    1-包含开始和结束时间  2-包含结束-不包含开始时间  3-包含开始-不包含结束时间  4-不包含开始和结束时间
     *  java.util.List<com.lplb.common.utils.LocalDateTimeUtil.DateDays>
     * @author wangsong
     */
    ...
    public static List<LocalDateTime> getBetweenDaysList(LocalDateTime startTime, LocalDateTime endTime, Integer type) {
        ...
        if (type == BETWEEN_TYPE_TWO) {
            ...
            oldEndTime = minus(endTime, 1, ChronoUnit.DAYS);
    ...
    /**
     * 计算两个日期的时间差并转化为文字描述 即x天x小时x分x秒
     *  from
     *  to
     *
     */
    public static String getTimeDifferenceTransferDesc(LocalDateTime from, LocalDateTime to) {
        long timeDifferenceSecond = ChronoUnit.SECONDS.between(from, to);
        int[] timeUnitNumArr = getTimeUnitNumArr(timeDifferenceSecond);
        StringBuilder builder = new StringBuilder();
        if (timeUnitNumArr[0]!=0){
            builder.append(timeUnitNumArr[0]).append("天");
        }
        if (timeUnitNumArr[1]!=0){
            builder.append(timeUnitNumArr[1]).append("小时");
        }
        if (timeUnitNumArr[2]!=0){
            builder.append(timeUnitNumArr[2]).append("分钟");
        }
        if (timeUnitNumArr[3]!=0){
            builder.append(timeUnitNumArr[3]).append("秒");
        }
        return builder.toString();
    }

    private static int[] getTimeUnitNumArr(long timeDifferenceSecond) {
        //存放天、时、分、秒的值
        int[] timeUnitArr = new int[4];
        if (timeDifferenceSecond<60){
            timeUnitArr[3] = (int) timeDifferenceSecond;
        }
        if (timeDifferenceSecond<3600){
            long minuteNum = timeDifferenceSecond / 60;
            long secondNum = timeDifferenceSecond % 60;
            timeUnitArr[2] = (int) minuteNum;
            timeUnitArr[3] = (int) secondNum;
        }
        if (timeDifferenceSecond<86400){
            long hourNum = timeDifferenceSecond / 3600;
            long minuteNum = (timeDifferenceSecond % 3600)/60;
            long secondNum = (timeDifferenceSecond % 3600)%60;
            timeUnitArr[1] = (int) hourNum;
            timeUnitArr[2] = (int) minuteNum;
            timeUnitArr[3] = (int) secondNum;
        }
        long dayNum = timeDifferenceSecond / 86400;
        long remainder = timeDifferenceSecond % 86400;
        long hourNum = remainder / 3600;
        long minuteNum = (remainder % 3600)/60;
        long secondNum = (remainder % 3600)%60;
        timeUnitArr[0] = (int) dayNum;
        timeUnitArr[1] = (int) hourNum;
        timeUnitArr[2] = (int) minuteNum;
        timeUnitArr[3] = (int) secondNum;
        return timeUnitArr;
    ...
    public static LocalDate parseToLocalDate(String dataTimeStr) {
        Set<Pattern> patterns = DATE_FORMATTER_PATTERN_MAP.keySet();
        String formatPattern = null;
        for (Pattern pattern:patterns) {
            if (pattern.matcher(dataTimeStr).matches()){
                formatPattern = DATE_FORMATTER_PATTERN_MAP.get(pattern);
                break;
            }
        }
        if (formatPattern==null && dataTimeStr.contains("Q")){
            //这种属于是按季度的执行日期，先不做data日期的转换了
            return LocalDate.now();
        }
        switch (formatPattern){
            case YYYY_MM:
                dataTimeStr = dataTimeStr+"-01";
                break;
            case YYYY_MM_DD:
                break;
        ...
        return LocalDate.parse(dataTimeStr);
    }

    public static String parseLd(LocalDate localDate) {
    ...
    public static int getWeekOfYear(LocalDate date) {
        TemporalField temporalField = WeekFields.of(DayOfWeek.MONDAY, 4).weekOfWeekBasedYear();
        return date.get(temporalField);
    }

    /**
     * 根据指定的年份和周数，获取该周的开始日期（默认为周一）。
     * @param year 指定的年份
     * @param weekOfWeekBasedYear 指定的周数
     * @return 该周的开始日期
     */
    public static LocalDate getStartDateOfWeek(int year, int weekOfWeekBasedYear) {
        WeekFields weekFields = WeekFields.of(DayOfWeek.MONDAY, 4);
        LocalDate startOfYear = LocalDate.of(year, 1, 1);
        LocalDate dateOfWeek = startOfYear.with(weekFields.weekOfYear(), weekOfWeekBasedYear);
        return dateOfWeek.with(weekFields.dayOfWeek(), 1);
    }
